...
function SellerProducts() {
  ...
  const save = async () => {
    ...
    const { error } = id
      ...
      : await supabase.from("products").insert({ ...rest, seller_id: user.id });
    ...
    <div className="p-8">
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">{products.length} listed</p>
        </div>
      ...
      <div className="rounded-xl border border-border bg-card overflow-hidden">
        <Table>
          ...
          <TableBody>
            {products.length === 0 ? (
              <TableRow><TableCell colSpan={6} className="py-12 text-center text-muted-foreground">No products yet. Click "Add product" to list your first item.</TableCell></TableRow>
            ) : products.map((p: any) => (
              <TableRow key={p.id}>
